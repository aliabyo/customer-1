...
dance_group = set()
moderators = ['Alionardo_','Sampire_',"1_on_1:62b272df4de9bb2d9d04d01c:650ce6dd22d153e5f0ee1764"]
...
class Bot(BaseBot):
    ...
    def load_moderators(self):
        ...
        default_moderators = ['Alionardo_', 'Rocketbunny187']
    ...
    async def on_user_join(self, user: User) -> None:
     ...
     try:
        ...
        await self.highrise.chat(f"Welcome to what gonna be your lovely place\n           Adults CLUB ™ 🥃\n for bots dm @Alionardo_  ")
    ...
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:
     try:
      if user.username == "Alionardo_" :
        ...
        if user.username == "Alionardo_" or user.username == "RocketBunny187" :
          if reaction == "wink":
      ...
      if user.username in ["Alionardo_", "RocketBunny187"] and reaction == "clap":
      ...
      if user.username in ["Alionardo_"] and reaction == "wave":
      ...
      try:
         ...
         if message == "!tip5":
              if user.username == "Alionardo_" :
                ...
                await  self.highrise.send_whisper(user.id, f"Only @Alionardo_ can use tip!")
         ...
         if message == "!tip1":
              if user.username == "Alionardo_":
                ...
                await  self.highrise.send_whisper(user.id, f"Only the @Alionardo_ can use tip!")
         ...
         if message.lower().lstrip().startswith(("-emote", "!emote")):
                ...
                await self.highrise.send_whisper(user.id, "\n•  Note that these commands will only work in room Adults CLUB ™ 🥃 . some expressions may not work because of restrictions.")
         ...
         if message.lstrip().startswith(("!vip","!giveVip","!0","!tip1", "!dj","!tip5","tip10","!tip50","!tip500"  )):
            response = await self.highrise.get_room_users()
            ...
            try:
                if message.startswith("!vip"):   
                  if user.username.lower() in self.moderators:
                    await self.highrise.teleport(user_id, Position(4.5, 15.5,6.5))
                ...
                if message.startswith("!dj"):   
                  if user.username.lower() in self.moderators:
                    await self.highrise.teleport(user_id, Position(10.5,2,5.5))
                if message.startswith("!0"):   
                  if user.username.lower() in self.moderators:
                     await self.highrise.teleport(user_id, Position(8, 0.0,20))
               
                   
                    
                if message.startswith("!tip1") and user.username  == "Alionardo_ " :
                    await self.highrise.tip_user(user_id, "gold_bar_1")
                if message.startswith("!tip5") and user.username  == "Alionardo_ ":
                    await self.highrise.tip_user(user_id, "gold_bar_5")
                if message.startswith("!tip10") and user.username  == "Alionardo_ " :
                  await self.highrise.tip_user(user_id, "gold_bar_10")
                if message.startswith("!tip50") and user.username  == "Alionardo_ ":
                  await self.highrise.tip_user(user_id, "gold_bar_50")
                if message.startswith("!tip500") and user.username  == "Alionardo_ " :
         ...
         if message.startswith('vip') :
            if user.username.lower() in self.moderators or user.username.lower() in self.membership :    
              await self.highrise.teleport(f"{user.id}", Position(4.5, 15.5,6.5))
         if message.startswith('dj') :
              if user.username.lower() in self.moderators:    
                await self.highrise.teleport(f"{user.id}", Position(10.5,2,5.5))

         if message.startswith('-0'):
              await self.highrise.teleport(f"{user.id}", Position(15.5, 0.5,15.5))
         if message.startswith('-1'):
              await self.highrise.teleport(f"{user.id}", Position(13.5, 6.25,19.5))
         if message.startswith('-2'):
             if user.username.lower() in self.moderators or user.username.lower() in self.membership :    
                await self.highrise.teleport(f"{user.id}", Position(13.5, 14.5,19.5))
             else:
         ...
         if message.startswith("/e1"):
          ...
          Item(type='clothing',
                    amount=1,
                    id='shirt-n_2016fallblackkknottedtee',
                   account_bound=False,
                    active_palette=-1),
          Item(type='clothing', 
               amount=1, 
               id='pants-n_room32019highwasittrackshortsblack',
               account_bound=False,
               active_palette=-1),
          Item(type='clothing', 
               amount=1, 
         ...
         Item(type='clothing',
              amount=1, 
              id='shoes-n_room12019sneakerspink',
              account_bound=False,
              active_palette=-1),      
         Item(type='clothing',
              amount=1, 
              id='freckl-n_sharpfaceshadow',
              account_bound=False,
              active_palette=-1),
         Item(type='clothing',
              amount=1, 
         ...
         Item(type='clothing',
              amount=1,
              id= 'hair_front-n_basic2018topknotwithbangs',
              account_bound=False, active_palette=2),       
         Item(type='clothing', 
              amount=1, 
              id='eye-n_basic2018heavymascera',
              account_bound=False,
              active_palette=2),
        Item(type='clothing', 
             amount=1,
         ...
         dance_emotes = {
           ...
           "blush": "emote-shy2",
           "anime": "dance-anime",
           "lolguitar": "idle-guitar",
           "head": "emote-headblowup",
           "zero": "emote-astronaut",
           "sayso": "idle-dance-tiktok4",
           "creepy": "emote-creepycute",
           "uwu": "idle-uwu",
           "wrong": "dance-wrong",
           "icecream": "dance-icecream",
           "gravity": "emote-gravity",
    ...
    async def on_message(self, user_id: str, conversation_id: str, is_new_conversation: bool) -> None:
        _bid = "12bb31fd732e0b449a3edfa7cbc705deff0bb54e2068966c18466039d237dc10"
        _id = f"1_on_1:{_bid}:{user_id}"
        _idx = f"1_on_1:{user_id}:{_bid}"
        _rid = "65c0fa17669747e73f9473b8" 
        ...
        if message.lower().lstrip().startswith(("invite fans","bring my", "bring fans")) and conversation_id in moderators:
          ...
          for conversation_id in self.fans:
              ...
              __rid = "65c0fa17669747e73f9473b8"
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem) -> None:
        try:
            ...
            if receiver.username  == "Leo_nardo__":
              if tip.amount == 500:
    ...
    async def on_user_move(self, user: User, pos: Position) -> None:
         ...
         if pos.x >= 6.5 and pos.x <=11.5:
             if pos.z >= 13.5 and pos.z <= 18.5:
               if pos.y >= 0 and pos.y <= 1:
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      try:
        ...
        self.highrise.tg.create_task(self.highrise.walk_to(AnchorPosition(entity_id='65ae2b54000000000000003f', anchor_ix=1)))
        self.load_temporary_vips()
